...
память в следующих интервью по той же теме. Telegram-пользователи работают в
общей коллекции знаний, а каждый результат сохраняет автора, исходную цитату и
workspace/topic boundary.
...
flowchart TD
    ...
    H5 --> J["draft_review<br/>ничего ещё не опубликовано"]
    A -->|/finish| J
    J --> J1["/edit /delete /add<br/>проверка экспертом"]
    J1 -->|/approve| K["durable postprocess_job<br/>lease + retry + heartbeat"]
    K --> L["worker + advisory lock<br/>src.jobs.extract"]
    L --> L1["grounded claims + evidence<br/>dedup + verified relations"]
    L1 --> M["пересборка сводки<br/>src.jobs.summary"]
...
После `/finish` или `end_session` создаётся draft. Эксперт проверяет его через
`/review`, `/edit`, `/delete`, `/add`; обработка запускается только после
`/approve`:

```mermaid
flowchart LR
    A["approved messages"] --> B["durable job<br/>queued → running → succeeded"]
    B --> B1["разбиение на чанки<br/>перехлест 2 сообщения"]
    B1 --> C["модель предлагает факты<br/>сырые кандидаты"]
    ...
    K --> K1["memory_claims + evidence<br/>verified one-hop relations"]
    K1 --> M["сессия получает<br/>статус extracted"]
    L --> M
    H --> M
    I --> M
...
Compose может запустить Postgres и бота. Chat/embedding endpoints остаются на
хосте и доступны контейнеру через `host.docker.internal`:

```bash
cp .env.example .env
# заполнить TELEGRAM_BOT_TOKEN и модели
# host-run bot читает OPENAI_BASE_URL/EMBED_BASE_URL
# Compose читает DOCKER_OPENAI_BASE_URL/DOCKER_EMBED_BASE_URL
docker compose up -d --build
```

Внутри контейнера `localhost` указывает на сам контейнер. Для сервисов на хосте:
...
`docker-compose.yml` сам переопределяет `DATABASE_URL` на хост `db` и проверяет,
что chat/embedding URL не loopback.
...
`DOCKER_EMBED_MODE=disabled` или внешний endpoint с
`DOCKER_EMBED_MODE=external`.
...
| `PUBLIC_COLLECTION_SLUG` | нет | общий workspace Telegram-пользователей, по умолчанию `public` |
| `ADMIN_TELEGRAM_USER_IDS` | нет | Telegram ID с доступом к `/verbose` |
| `HYBRID_RAG_ENABLED` | нет | переключает auto-RAG на Hybrid RAG после evaluation |
| `HYBRID_RAG_SHADOW` | нет | сравнивает Hybrid RAG с vector baseline без изменения ответов |
...
| Таблица | Что хранит |
|---------|------------|
| `users`, `workspaces`, `workspace_members` | пользователей, security boundary и роли |
| `topics` | темы внутри workspace |
...
| `review_items` | draft, правки и approved snapshot |
| `postprocess_jobs` | durable queue, lease, retry, idempotency и ошибки |
...
| `memory_claims` | текущие и исторические semantic claims |
| `memory_claim_evidence` | immutable evidence и distinct contributors |
| `memory_claim_relations` | verified `supports/contradicts/refines/depends_on` |
| `session_context_compactions` | persistent projection длинной истории |
...
python -m scripts.export <workspace_id> <topic_id> kb_topic.md
...
```sql
SELECT id, workspace_id, topic_id, topic, expert_name, status,
       tokens_used, created_at, finished_at
FROM sessions
ORDER BY id DESC
LIMIT 20;

SELECT ts.workspace_id, ts.topic_id, t.name AS topic,
       ts.generated_at, ts.summary
FROM topic_summaries ts
JOIN topics t ON t.id=ts.topic_id AND t.workspace_id=ts.workspace_id
ORDER BY ts.generated_at DESC;

SELECT e.id, e.workspace_id, e.topic_id, s.topic, e.type, e.origin, e.support_mode,
...
```sql
SELECT c.id, c.type, c.normalized_statement, c.payload,
       count(DISTINCT ce.user_id) AS contributors
FROM memory_claims c
JOIN memory_claim_evidence ce ON ce.claim_id=c.id
WHERE c.workspace_id=$1 AND c.topic_id=$2
  AND c.status='verified' AND c.valid_to IS NULL
GROUP BY c.id
ORDER BY c.id;
```
...
| `/finish` | перейти в `draft_review`; знания ещё не публикуются |
| `/review` | показать текущий draft |
| `/edit N текст` | исправить пункт draft |
| `/delete N` | удалить неверный пункт draft |
| `/add текст` | добавить пропущенный пункт |
| `/approve` | зафиксировать draft и атомарно создать durable job |
...
python -m scripts.eval_retrieval tests/fixtures/retrieval_golden.example.jsonl
...
Тесты используют `pytest` поверх стандартного `unittest`:
...
- workspace isolation, review/approve и durable job recovery;
- Hybrid RAG, evidence projection, distinct-user consensus и one-hop relations;
- context compaction без удаления raw history;
...
src/
  ...
  dialogue_context.py persistent compaction длинных интервью
  ...
  memory.py         claims, evidence, temporal state и entity aliases
  retrieval.py      vector + FTS + RRF + verified one-hop recall
  jobs/
    extract.py      транскрипт -> проверенные extracted_items
    worker.py       lease-based durable post-processing worker
    hierarchy.py    опциональные иерархические summaries
...
scripts/
  ...
  audit_memory.py   проверки tenant/evidence/job invariants
  backfill_memory.py idempotent projection старых verified items
  eval_retrieval.py evaluation Hybrid RAG на golden JSONL
...
- Полный entity GraphRAG намеренно не добавлен. Используется evidence-first claim
  graph в PostgreSQL и максимум один verified hop. Neo4j не нужен до измеренной
  потребности в сложных multi-hop обходах.

## Multi-user security и публикация

- Все Telegram users входят как `member` в общий workspace `public`, поэтому
  cross-user RAG и consensus реально работают. Legacy/CLI sessions остаются в
  изолированных workspaces.
- Workspace остаётся security boundary: одинаковые названия тем в разных
  workspaces не пересекаются, cross-workspace SQL/RAG/relations запрещены.
...
PostgreSQL остаётся единственным datastore. Источник истины — raw message,
точный evidence span и grounded claim; relation graph является проверяемой
проекцией, а не самостоятельной базой фактов.

Hybrid retrieval выполняет vector top-20 и PostgreSQL FTS top-20, объединяет их
через Reciprocal Rank Fusion, фильтрует workspace/status/version и возвращает
5–8 claims с цитатами. Verified `contradicts`, `supports`, `refines` и
`depends_on` могут добавить максимум один relation hop. Relation получает место
даже при полностью заполненном flat result limit.
...
Сначала используйте `HYBRID_RAG_SHADOW=true`. Включайте
`HYBRID_RAG_ENABLED=true` после выигрыша над baseline по evidence recall,
contradiction recall, citation correctness и latency. Подробности — в
